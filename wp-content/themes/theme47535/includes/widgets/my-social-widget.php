...
class My_SocialNetworksWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<?php foreach(array("Facebook", "Twitter", "Youtube", "Google+", "RSS") as $network) : ?>
			...
			<li class="social_li">
				<a class="social_link social_link__<?php echo strtolower($network); ?>" rel="tooltip" data-original-title="<?php echo strtolower($network); ?>" href="<?php echo $networks[$network]['link']; ?>">
					<?php if (($display == "both") or ($display =="icons")) { ?>
						<span class="social_ico"><i class="icon-<?php if ($network=='Google+') { $network = 'google-plus'; } echo strtolower($network);?>-sign"></i></span>
					<?php } if (($display == "labels") or ($display == "both")) { ?> 
						<span class="social_label"><?php if ($network=='google-plus') { $network = 'Google+'; } if (($networks[$network]['label'])!=="") { echo $networks[$network]['label']; } else { echo $network; } ?></span>
					<?php } ?>
				</a>
			</li>
			<?php endif; ?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'twitter' => '', 'twitter_label' => '', 'facebook' => '', 'facebook_label' => '', 'rss' => '', 'rss_label' => '', 'youtube' => '', 'youtube_label' => '', 'google' => '', 'google_label' => '', 'display' => 'icons', 'text' => '');
...
?>
	...
	<p><label for="<?php echo $this->get_field_id('rss'); ?>"><?php echo 'RSS feed:' ?></label>
